...
class UserRideRequestDataTable extends DataTable
{
    /**
        ...
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($rideRequest){
                return '<div class="hstack gap-2 flex-wrap">
                            <a href="'.route('ride-requests.show',$rideRequest).'" class="d-block mb-3 text-primary fs-14 lh-1"><i class="ri-eye-2-line"></i></a>
                            <a href="javascript:void(0);" data-href="'.route('ride-requests.edit',$rideRequest).'" class="edit-model text-info fs-14 lh-1"><i class="ri-edit-line"></i> Edit</a>
    ...
    {
        return $this->builder()
                    ->setTableId('userriderequest-table')
    ...
    public function customHtml($user): HtmlBuilder
    {
        return $this->builder()
            ->setTableId('userriderequest-table')
            ->columns($this->getColumns())
            ->minifiedAjax(route('users.rideRequest.index',$user))
            ->orderBy(1)
            ->responsive(true)
            ->selectStyleSingle()
            ->buttons([
    ...
    {
        return 'UserRideRequest_' . date('YmdHis');
    }
}

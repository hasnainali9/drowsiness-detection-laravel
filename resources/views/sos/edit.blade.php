...
<div class="modal-body">
    ...
    <form method="POST" action="{{ route('sos-list.update',$sos) }}">
        @csrf
        @method('PUT')
        <div class="col-xl-12 mb-3">
            ...
            <select class="form-control select_2_picker" name="user_id" required>
                ...
                @foreach(\App\Models\User::all() as $user)
                    <option value="{{$user->id}}" @if($sos->user_id==$user->id) selected @endif>{{$user->name}} ({{$user->email}})</option>
                @endforeach
            </select>
            @error('name')
        ...
        <div class="col-xl-12 mb-3">
            ...
            <input id="phone_no" type="tel" class="form-control" value="{{$sos->phone_no}}" required />
            <input type="hidden" name="phone_no" id="phone_no_full" value="{{$sos->phone_no}}" required />
            @error('phone_no')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
        ...
        <div class="form-check mb-2">
            <input class="form-check-input" type="checkbox" name="message" id="message" @if($sos->message) checked @endif >
        ...
        <div class="form-check mb-2">
            <input class="form-check-input" type="checkbox" name="call" id="call" @if($sos->call) checked @endif >
